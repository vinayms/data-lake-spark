...
config.read('dl.cfg')
os.environ['AWS_ACCESS_KEY_ID']=config['AWS_KEY']['AWS_SECRET_ACCESS_KEY']
...
def process_song_data(spark, input_data, output_data):
    ...
    # read song data file
    song_data_df = spark.read.json(input_data+"song_data/*/*/*/*.json")
    ...
    artists_table = song_data_df['artist_id','artist_name','artist_location','artist_latitude','artist_longitude'].dropDuplicates(['artist_id'])
...
def process_log_data(spark, input_data, output_data):
    ...
    user_table = log_data_df['userId','firstName','lastName','gender','level'].where(filter).dropDuplicates(['userId'])
    print('user table count - '+str(user_table.count()))
    ...
    print('time table count - '+str(time_table.count()))
    ...
    songplays_table = log_data_df.select(
...
def main():
    ...
    output_data = "s3a://vin-udacity-data-lake-spark/"
